...
## Q1
**Task:** Q1: Random Forest and J48 on iris.arff compare.
...
## Q2
**Task:** Q2: Regression using logistic regression OR K-Means/Hierarchical on Credit Card dataset (elbow).
...
### Regression tasks:
1. Preprocess → Open file → select regression dataset (e.g., airline or appropriate CSV/ARFF).
2. Classify tab → choose LinearRegression (functions → LinearRegression) or Logistic.
3. For regression, use Cross-validation and observe RMSE, MAE, and correlation coefficient listed in output.
4. Report accuracy in context (for regression: lower RMSE/MAE is better).
