...
## Q1
**Task:** Q1: Replace missing values in weather.arff and analyze effect.
...
## Q2
**Task:** Q2: Scale features in iris.arff between 0 and 1 and compare statistics OR Apriori on weather.nominal.arff (top 3 rules).

### Association Rules (Apriori):
1. Explorer → Preprocess → Open file → weather.nominal.arff or supermarket.arff.
2. Go to 'Associate' tab.
3. Choose 'Apriori'.
4. Set minimum support and confidence as required (eg support=0.1 default). For Qs that ask change support to 0.5, set it and observe number of rules.
5. Click 'Start'. Observe top rules printed and their support/confidence/lift.
6. Interpret top 3 rules: list LHS => RHS, support, confidence, and business meaning.


### Data Preprocessing (Missing values / Scaling / Normalize):
1. Preprocess → Open file → choose dataset (weather.arff or iris.arff).
2. Under 'Filter', choose unsupervised → attribute → ReplaceMissingValues to fill missing data, or unsupervised → attribute → Normalize to scale to [0,1].
3. Click 'Apply'. Compare attribute stats before and after (use 'Visualize' or 'Dataset summary').
4. If evaluation required, run your classifier on the original and preprocessed dataset and compare metrics.
